...
double GridPathPlanning::getCosts(const GridNode * const currentNode, const GridNode * const successorNode) const {
	...
	result = double(sqrt(pow(currentNode->x - successorNode->x, 2) + pow(currentNode->y - successorNode->y, 2)));
	return result;
}
...
double StraightLineDistanceHeuristic::heuristic(const GridNode* const currentNode, const GridNode* const goalNode) const {
	...
	result = double(sqrt(pow(currentNode->x - goalNode->x, 2) + pow(currentNode->y - goalNode->y, 2)));
	return result;
}
...
std::vector<AbstractNode *> GridPathPlanning::getNeighborNodes(const GridNode * const currentNode, const GridMap& map) {
	 ...
	 * - map.isOccupied(int x, int y): returns true if the cell is occupied by an obstacle
	...
	for(int i = -1; i <= 1; i++){
		for(int j = -1; j <= 1; j++) {
			if(i == 0 && j == 0)
				continue;
			if(currentNode->x + i >= 0 && currentNode->x +i < map.width && currentNode->y +j >= 0 && currentNode->y + j < map.height)
			{
